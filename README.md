...
ダイナミック・スコープです。けれども、昔の Common Lisp における「#'」が使えます。
これは、ラムダ項の前に付けて関数クロージャを生成することによって、一時的にレキシカル・スコープを導入することです。
...
ラムダ項を引数に取って、funarg 式を生成します。

### funcall

当初理系においては、関数を引数に直接に渡すことができません。
そのためには fucall を使います。
ここらへんのところは、Common Lisp と共通です。
