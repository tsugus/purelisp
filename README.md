...
すなわち、car セルがシンボルを指しているドット対の成すリストです。
...
これも LISPer なら知っていて当然でしょう。当処理系においては、これは単なる印としてのシンボルです。
...
そうでないときは、シンボル内部に値を格納します。当処理系においては、このようなシンボルは oblis というリストに登録され、ガベージ・コレクションによって消去されなくなります。（ただし、リストから削除する関数は作ってありません。不便なのは何となくわかっているのですが。そうする必要に迫られるほど使っていないので。）

ちなみに当処理系においては、ラムダ式の前にクォートを付けたものをシンボルに代入すると、そのシンボルが関数そのものになります。
...
condition は何らかの S-式です。
condition を評価し他結果が空リスト**以外になる**まで、until は式 s_1, s_2, ... s_n を順次実行することを繰り返します。
最後の S_n が実行された結果が、全体の返す結果となります。
